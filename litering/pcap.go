...
// USING: https://godoc.org/github.com/wheelcomplex/gopacket/pcapgo for pcap file io
...
// Handle provides a connection to a litering handle, allowing users to read packets
...
//
// Handles are already litering_activate'd
type Handle struct {
	// cptr is the handle for the actual litering C object.
	cptr         *C.litering_t
	blockForever bool
	device       string
	mu           sync.Mutex
	...
	pkthdr  *C.struct_litering_pkthdr
	buf_ptr *C.u_char
}

// Stats contains statistics on how many packets were handled by a litering handle,
...
func OpenLive(device string, snaplen int32, promisc bool, timeout time.Duration) (handle *Handle, _ error) {
	...
	p.cptr = C.litering_open_live(dev, C.int(snaplen), pro, timeoutMillis(timeout), buf)
	if p.cptr == nil {
...
func OpenOffline(file string) (handle *Handle, err error) {
	...
	cptr := C.litering_open_offline(cf, buf)
	if cptr == nil {
		return nil, errors.New(C.GoString(buf))
	}
...
// NextError returns the next packet read from the litering handle, along with an error
...
func (p *Handle) getNextBufPtrLocked(ci *gopacket.CaptureInfo) error {
	...
	for {
		result = NextError(C.litering_next_ex(p.cptr, &p.pkthdr, &p.buf_ptr))
...
// Close closes the underlying litering handle.
func (p *Handle) Close() {
	C.litering_close(p.cptr)
}

// Error returns the current error associated with a litering handle (litering_geterr).
func (p *Handle) Error() error {
	return errors.New(C.GoString(C.litering_geterr(p.cptr)))
}

// Stats returns statistics on the underlying litering handle.
func (p *Handle) Stats() (stat *Stats, err error) {
	var cstats _Ctype_struct_litering_stat
	if -1 == C.litering_stats(p.cptr, &cstats) {
		return nil, p.Error()
	}
	return &Stats{
...
func (p *Handle) ListDataLinks() (datalinks []Datalink, err error) {
	...
	n := int(C.litering_list_datalinks(p.cptr, &dlt_buf))
	if -1 == n {
		return nil, p.Error()
	}

	defer C.litering_free_datalinks(dlt_buf)
	...
	for i := 0; i < n; i++ {
		expr := C.litering_datalink_val_to_name((*dltArray)[i])
		datalinks[i].Name = C.GoString(expr)

		expr = C.litering_datalink_val_to_description((*dltArray)[i])
...
// SetBPFFilter compiles and sets a BPF filter for the litering handle.
func (p *Handle) SetBPFFilter(expr string) (err error) {
	...
	// No device indicates we're handling a litering file.
	if len(p.device) > 0 {
		dev := C.CString(p.device)
		defer C.free(unsafe.Pointer(dev))
		if -1 == C.litering_lookupnet(
			dev,
	...
	if -1 == C.litering_compile(p.cptr, &bpf, cexpr, 1, C.bpf_u_int32(maskp)) {
		return p.Error()
	}

	if -1 == C.litering_setfilter(p.cptr, &bpf) {
		C.litering_freecode(&bpf)
		return p.Error()
	}
...
func (p *Handle) NewBPF(expr string) (*BPF, error) {
	...
	if C.litering_compile(p.cptr, &bpf.bpf, cexpr /* optimize */, 1, C.PCAP_NETMASK_UNKNOWN) != 0 {
		return nil, p.Error()
	}
...
func (b *BPF) Matches(ci gopacket.CaptureInfo, data []byte) bool {
	var hdr C.struct_litering_pkthdr
	...
	return C.litering_offline_filter(&b.bpf, &hdr, dataptr) != 0
}

// Version returns litering_lib_version.
func Version() string {
	return C.GoString(C.litering_lib_version())
}

// LinkType returns litering_datalink, as a layers.LinkType.
func (p *Handle) LinkType() layers.LinkType {
	return layers.LinkType(C.litering_datalink(p.cptr))
}

// SetLinkType calls litering_set_datalink on the litering handle.
func (p *Handle) SetLinkType(dlt layers.LinkType) error {
	if -1 == C.litering_set_datalink(p.cptr, C.int(dlt)) {
		return p.Error()
	}
	return nil
}
...
func FindAllDevs() (ifs []Interface, err error) {
	...
	buf = (*C.char)(C.calloc(errorBufferSize, 1))
	defer C.free(unsafe.Pointer(buf))
	var alldevsp *C.litering_if_t

	if -1 == C.litering_findalldevs((**C.litering_if_t)(&alldevsp), buf) {
		return nil, errors.New(C.GoString(buf))
	}
	defer C.litering_freealldevs((*C.litering_if_t)(alldevsp))
	dev := alldevsp
	var i uint32
	for i = 0; dev != nil; dev = (*C.litering_if_t)(dev.next) {
		i++
	}
	ifs = make([]Interface, i)
	dev = alldevsp
	for j := uint32(0); dev != nil; dev = (*C.litering_if_t)(dev.next) {
		var iface Interface
...
func findalladdresses(addresses *_Ctype_struct_litering_addr) (retval []InterfaceAddress) {
	// TODO - make it support more than IPv4 and IPv6?
	retval = make([]InterfaceAddress, 0, 1)
	for curaddr := addresses; curaddr != nil; curaddr = (*_Ctype_struct_litering_addr)(curaddr.next) {
		// Strangely, it appears that in some cases, we get a litering address back from
		// litering_findalldevs with a nil .addr.  It appears that we can skip over
		// these.
		if curaddr.addr == nil {
			continue
		}
...
// WritePacketData calls litering_sendpacket, injecting the given data into the litering handle.
func (p *Handle) WritePacketData(data []byte) (err error) {
	if -1 == C.litering_sendpacket(p.cptr, (*C.u_char)(&data[0]), (C.int)(len(data))) {
		err = p.Error()
	}
	return
}
...
func (p *Handle) SetDirection(direction Direction) error {
	...
	if status := C.litering_setdirection(p.cptr, (C.litering_direction_t)(direction)); status < 0 {
...
func (t TimestampSource) String() string {
	return C.GoString(C.litering_tstamp_type_val_to_name(C.int(t)))
}
...
func TimestampSourceFromString(s string) (TimestampSource, error) {
	t := C.litering_tstamp_type_name_to_val(C.CString(s))
	if t < 0 {
...
func statusError(status C.int) error {
	return errors.New(C.GoString(C.litering_statustostr(status)))
}

// InactiveHandle allows you to call pre-litering_activate functions on your litering
...
func (p *InactiveHandle) Activate() (*Handle, error) {
	err := activateError(C.litering_activate(p.cptr))
	if err != aeNoError {
		return nil, err
	}
...
func NewInactiveHandle(device string) (*InactiveHandle, error) {
	...
	// This copies a bunch of the litering_open_live implementation from litering.c:
	cptr := C.litering_create(dev, buf)
	if cptr == nil {
		return nil, errors.New(C.GoString(buf))
	}
...
func (p *InactiveHandle) SetSnapLen(snaplen int) error {
	if status := C.litering_set_snaplen(p.cptr, C.int(snaplen)); status < 0 {
		return statusError(status)
	}
	return nil
}
...
func (p *InactiveHandle) SetPromisc(promisc bool) error {
	...
	if status := C.litering_set_promisc(p.cptr, pro); status < 0 {
		return statusError(status)
	}
	return nil
}
...
func (p *InactiveHandle) SetTimeout(timeout time.Duration) error {
	p.blockForever = timeout < 0
	if status := C.litering_set_timeout(p.cptr, timeoutMillis(timeout)); status < 0 {
		return statusError(status)
	}
	return nil
}
...
func (p *InactiveHandle) SupportedTimestamps() (out []TimestampSource) {
	var types *C.int
	n := int(C.litering_list_tstamp_types(p.cptr, &types))
	defer C.litering_free_tstamp_types(types)
...
func (p *InactiveHandle) SetTimestampSource(t TimestampSource) error {
	if status := C.litering_set_tstamp_type(p.cptr, C.int(t)); status < 0 {
		return statusError(status)
	}
	return nil
}
...
// setting RFMon because litering_can_set_rfmon returns 0.
...
func (p *InactiveHandle) SetRFMon(monitor bool) error {
	...
	switch canset := C.litering_can_set_rfmon(p.cptr); canset {
	case 0:
	...
	if status := C.litering_set_rfmon(p.cptr, mon); status != 0 {
		return statusError(status)
	}
	return nil
}
...
func (p *InactiveHandle) SetBufferSize(bufferSize int) error {
	if status := C.litering_set_buffer_size(p.cptr, C.int(bufferSize)); status < 0 {
		return statusError(status)
	}
	return nil
}
...
func (p *InactiveHandle) SetImmediateMode(mode bool) error {
	...
	if status := C.litering_set_immediate_mode(p.cptr, md); status < 0 {
		return statusError(status)
	}
	return nil
}
